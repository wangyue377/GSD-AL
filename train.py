...
def parse_args():
    ...
    parser.add_argument('--config',
                        default='configs/redet/redet_re50_refpn_1x_dota_le90.py',
    ...
    parser.add_argument('--class-num',
                        default=15,
                        help="class num: DOTA-v1.0(15) and DIOR-R(20)")
    parser.add_argument('--al-sample',
                        default='udc',
                        choices=['mus-cdb', 'usgss', 'ssl', 'udc'],
                        )
...
def main():
    ...
    if args.launcher == 'none':
        ...
        # 新增部分：将整数类型的 cfg.gpu_ids 转换为 torch.device 对象列表
        cfg.gpu_ids = [torch.device(f"cuda:{gpu}") if gpu >= 0 else torch.device("cpu") for gpu in cfg.gpu_ids]
    ...
    if cycle < 7:
        intermediate_model_path = osp.join(cfg.work_dir, 'intermediate_model.pth')
        torch.save(model.state_dict(), intermediate_model_path)
        intermediate_X_L_path = osp.join(cfg.work_dir, 'intermediate_X_L.npy')
        np.save(intermediate_X_L_path, X_L)
        intermediate_X_U_path = osp.join(cfg.work_dir, 'intermediate_X_U.npy')
        np.save(intermediate_X_U_path, X_U)
        ...
        # 调整相似度阈值动态衰减
        similarity_threshold = max(0.65, 0.8 - 0.05 * cycle)  # 每轮降低0.05
        ...
        selectionmethod.select(args.budget, args.iou_thresh, label_num,
                               args.score_thresh, args.class_num, similarity_threshold)
        # get the config of the labeled dataset
