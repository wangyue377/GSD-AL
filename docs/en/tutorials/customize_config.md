...
- Update keys inside a list of configs.
  ...
  normally a list
  e.g. `[dict(type='LoadImageFromFile'), ...]`. If you want to change `'LoadImageFromFile'` to `'LoadImageFromWebcam'`
  in the pipeline,
...
- `[norm_setting]`: `bn` (Batch Normalization) is used unless specified, other norm layer types could be `gn` (Group
...
It's worth noting that when modifying intermediate variables in the children configs, the user needs to pass the
intermediate variables into corresponding fields again.
For example, we would like to use an offline multi-scale strategy to train an RoI-Trans. `train_pipeline` are
