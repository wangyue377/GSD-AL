...
class RotatedRPNHead(AnchorHead):
    ...
    def _get_targets_single(self,
        ...
        num_valid_anchors = anchors.shape[0]  # 238950
        bbox_targets = torch.zeros_like(anchors)  # torch.Size([238950, 4])
        bbox_weights = torch.zeros_like(anchors)  # torch.Size([238950, 4])
        labels = anchors.new_full((num_valid_anchors,),  # torch.Size([238950])
                                  self.num_classes,  # 默认值为self.num_classes=15
                                  dtype=torch.long)
        label_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float)  # torch.Size([238950])

        pos_inds = sampling_result.pos_inds  # pos index
        neg_inds = sampling_result.neg_inds  # neg index
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox:
                pos_bbox_targets = self.bbox_coder.encode(  # deviate
                    sampling_result.pos_bboxes, sampling_result.pos_gt_bboxes)
            else:
                pos_bbox_targets = sampling_result.pos_gt_bboxes
            bbox_targets[pos_inds, :] = pos_bbox_targets  # deviate
    ...
    def get_targets(self,
        ...
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]]  # [196608, 49152, 12288, 3072, 768]
        ...
        labels_list = images_to_levels(all_labels, num_level_anchors)  # 只有0和1组成，且1数量非常多
    ...
    def loss(self,
        ...
        cls_reg_targets = self.get_targets(  # important(anchors to compute rpn_loss)
            anchor_list,
            valid_flag_list,
            gt_bboxes,
            img_metas,
        ...
        num_total_samples = (
            num_total_pos + num_total_neg if self.sampling else num_total_pos)  # 512(256*2)(2=batch size)
    ...
    def _get_bboxes_single(self,
        ...
        proposals = self.bbox_coder.decode(  # x1,y1,x2,y2....
