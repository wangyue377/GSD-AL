...
class RotatedBBoxHead(BaseModule):
    ...
    def loss(self,
        ...
        Args:
            cls_score (torch.Tensor): Box scores, has shape
                (num_boxes, num_classes + 1).                       # 每个框预测的类别概率分布[1024, 16]
            bbox_pred (Tensor, optional): Box energies / deltas.
                has shape (num_boxes, num_classes * 5).             # 每个框预测的针对每个类别的位置
            rois (torch.Tensor): Boxes to be transformed. Has shape
                (num_boxes, 5). last dimension 5 arrange as
                (batch_index, x1, y1, x2, y2).                      # 等待去移动的rois（RPN生成）
            labels (torch.Tensor): Shape (n*bs, ).                  # 目标   n??  bs??
            label_weights(torch.Tensor): Labels_weights for all     # weight用于区分正负示例
                  proposals, has shape (num_proposals,).
            bbox_targets(torch.Tensor):Regression target for all
                  proposals, has shape (num_proposals, 5), the
                  last dimension 5 represents [cx, cy, w, h, a].     # 目标
            bbox_weights (list[tensor],Tensor): Regression weights for   # weight用于区分正负示例，比如负示例不用回归学习
                  all proposals in a batch, each tensor in list has shape
                  (num_proposals, 5) when `concat=False`, otherwise just a
                  single tensor has shape (num_all_proposals, 5).
            reduction_override (str, optional): The reduction method used to  #覆盖损失??  (mean)
               override the original reduction method of the loss.
               Defaults to None.
        """
        losses = dict()
        if cls_score is not None:
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)  # 1024(所有proposal的数量)
    ...
    def get_bboxes(self,
        ...
        if cfg is None:
            ...
            # det_bboxes, det_labels, det_entropys, bg_scores = multiclass_nms_rotated(
            #     bboxes, scores, cfg.score_thr, cfg.nms, cfg.max_per_img)
            det_bboxes, det_labels, det_entropys, bg_scores, all_scores = multiclass_nms_rotated(
                bboxes, scores, cfg.score_thr, cfg.nms, rois[..., 1:], cfg.max_per_img)
            return det_bboxes, det_labels, det_entropys, bg_scores, bboxes, all_scores
