...
class RoITransRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        bbox_results = self._bbox_forward(stage, x,
                                          rois)  # predicted value for each roi:cls_score([1024,16]), bbox_pred([1024,5]), bbox_feats([1024,256,7,7])
        bbox_targets = self.bbox_head[stage].get_targets(  # gt for each roi
            sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg)
        if img_metas[0]['label_type'] == 'full':
        ...
        # # ======================= added ================================
        if img_metas[0]['label_type'] == 'partial':
            selected_bboxes = [res.bboxes for res in sampling_results]
            with torch.no_grad():
                bg_scores = self.simple_test(x, selected_bboxes, img_metas,
                                             return_bg_score=True)  # list(tensor*2)(512,)
                bg_score = torch.cat([_score for _score in bg_scores])
                assigned_label, _, _, _ = bbox_targets
                neg_inds = assigned_label == self.bbox_head[-1].num_classes
                bbox_targets[1][neg_inds.cpu().numpy()] = bg_score[neg_inds.cpu().numpy()]
            loss_bbox = self.bbox_head[stage].loss(bbox_results['cls_score'],
                                                   bbox_results['bbox_pred'], rois,
                                                   *bbox_targets)
            bbox_results.update(
    ...
    def simple_test(self, x, proposal_list, img_metas, return_split=False, return_entropy=False, return_bg_score=False,
                    return_aug_bbox_result=False, rescale=False):
        ...
        rois = bbox2roi(proposal_list)  # shape (n, 5), [batch_ind, x1, y1, x2, y2]
        for i in range(self.num_stages):
            bbox_results = self._bbox_forward(i, x, rois)
        ...
        if return_bg_score:
            bg_scores = []
            for i in range(num_imgs):
                _scores = F.softmax(cls_score[i], dim=-1) if cls_score[i] is not None else None
                bg_scores.append(_scores[:, -1])
            return bg_scores
        ...
        all_scores = []
        for i in range(num_imgs):
            det_bbox, det_label, det_entropys, bg_scores, bboxes, scores = self.bbox_head[-1].get_bboxes(
                rois[i],
                cls_score[i],
                bbox_pred[i],
                img_shapes[i],
        ...
        if return_aug_bbox_result:
            return bboxes, scores
        if return_split:
            return det_bboxes[0], det_labels[0], bg_scores
        if return_entropy:
            return det_bboxes[0], det_labels[0], det_entropys, bg_scores, all_scores[0]
