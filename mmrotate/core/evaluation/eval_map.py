...
def tpfp_default(det_bboxes,
    ...
    # 合并gt_bboxes和gt_bboxes_ignore
    if gt_bboxes.size > 0 and gt_bboxes_ignore.size > 0:
        gt_bboxes = np.vstack((gt_bboxes, gt_bboxes_ignore))
    elif gt_bboxes.size > 0:
        # 只有gt_bboxes
        pass
    elif gt_bboxes_ignore.size > 0:
        # 只有gt_bboxes_ignore
...
def get_cls_results(det_results, annotations, class_id):
    ...
    for ann in annotations:
        ...
        # 检查gt_inds的类型
        # print(f"gt_inds type: {type(gt_inds)}, value: {gt_inds}")
        # # 若为元组，转换为整数索引
        # gt_inds = gt_inds[0] if isinstance(gt_inds, tuple) else gt_inds
        # cls_gts.append(ann['bboxes'][gt_inds, :])
        # bboxes = np.array(ann['bboxes'])  # 强制转换为数组
        # cls_gts.append(bboxes[gt_inds, :])
        # 确保gt_bboxes是numpy数组
        gt_bboxes = np.array(ann['bboxes'])

        # 获取当前类别的gt bboxes
        cls_gt_bboxes = gt_bboxes[gt_inds]

        # 处理空的情况，确保形状为 (0, 5)
        if cls_gt_bboxes.size == 0:
            cls_gt_bboxes = np.zeros((0, 5), dtype=np.float32)

        cls_gts.append(cls_gt_bboxes)
        ...
        # 处理忽略的bboxes
        if 'bboxes_ignore' in ann and ann['bboxes_ignore'] is not None:
            ignore_inds = ann['labels_ignore'] == class_id
            gt_bboxes_ignore = np.array(ann['bboxes_ignore'])
            cls_gt_ignore_bboxes = gt_bboxes_ignore[ignore_inds]

            # 处理空的情况
            if cls_gt_ignore_bboxes.size == 0:
                cls_gt_ignore_bboxes = np.zeros((0, 5), dtype=np.float32)

            cls_gts_ignore.append(cls_gt_ignore_bboxes)
        else:
            # 确保空的忽略bboxes形状正确
